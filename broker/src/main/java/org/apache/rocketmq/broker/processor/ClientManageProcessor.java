...
public class ClientManageProcessor extends AsyncNettyRequestProcessor {
    ...
    //liqinglong: 处理所有【客户端】请求
    @Override
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request)
        throws RemotingCommandException {
        switch (request.getCode()) {
            //liqinglong: 处理客户端心跳数据
    ...
    //liqinglong: 处理客户端心跳数据
    //包括注册新的客户端、更新客户端数据、通知组内所有客户端以实现【客户端 rebalance】
    public RemotingCommand heartBeat(ChannelHandlerContext ctx, RemotingCommand request) {
        RemotingCommand response = RemotingCommand.createResponseCommand(null);
        HeartbeatData heartbeatData = HeartbeatData.decode(request.getBody(), HeartbeatData.class);
        //liqinglong: 将【远程通信信道信息】和【ClientID】映射
        ...
        for (ConsumerData data : heartbeatData.getConsumerDataSet()) {
            ...
            if (null != subscriptionGroupConfig) {
                //liqinglong: 【broker】端对【当客户端信息变化时是否主动通知组内所有客户端】的配置，默认为【true】
    ...
    //liqinglong: 校验客户端订阅信息中的消息过滤类型（SQL92）在【broker】中是否支持，以及【SQL92】格式是否正确
    public RemotingCommand checkClientConfig(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        if (requestBody != null && requestBody.getSubscriptionData() != null) {
            ...
            //liqinglong: 对于【tag】没有什么格式要求，直接放行
            if (ExpressionType.isTagType(subscriptionData.getExpressionType())) {
                response.setCode(ResponseCode.SUCCESS);
                response.setRemark(null);
                return response;
            }
            //liqinglong: 如果是【SQL92】，需要【broker】本身支持【enablePropertyFilter】，默认是 false
            if (!this.brokerController.getBrokerConfig().isEnablePropertyFilter()) {
                response.setCode(ResponseCode.SYSTEM_ERROR);
                response.setRemark("The broker does not support consumer to filter message by " + subscriptionData.getExpressionType());
                return response;
            }
            //liqinglong: 如果 broker 支持【SQL92】，则校验其格式
            try {
